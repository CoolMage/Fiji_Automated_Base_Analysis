...
- Kymograph tracking dependencies when using the Lumicks-based workflow:
  - `numpy`
  - `scikit-image`
  - `roifile`
  - `lumicks.pylake` (only required when choosing the Lumicks tracker)
...
## Kymograph tracking options

The pipeline can export tracked kymographs as ImageJ ROI ZIP archives. Two
tracking backends are available:

- **Lumicks greedy tracker** (default): powered by `lumicks.pylake` with
  `scikit-image` + `numpy` for TIFF loading and normalization. Install with:

  ```bash
  pip install numpy scikit-image roifile lumicks.pylake
  ```

- **KymographDirect**: runs the external KymographDirect executable and parses
  the resulting CSV output into ROIs. Provide the path to the executable in the
  GUI or when constructing `KymographProcessor` programmatically.

### CLI and GUI usage

- **CLI**: you can wire the kymograph processor into your own scripts via
  `KymographProcessor`. Kymographs generated by macros are saved under
  `<base>/<kymograph_output_folder>/<document>/<document>_kymo.tif` and the
  tracker writes ROIs to `<base>/<roi_output_folder>/<document>/<document>_kymo_tracks.zip`.
- **GUI**: the Kymograph tab lets you switch between **Lumicks** and
  **KymographDirect** methods, configure tracker thresholds, and choose the
  executable path for KymographDirect.
