...
def _collect_psf_paths(path_args: List[str]) -> List[str]:
    """Expand repeated or semicolon-separated PSF paths."""
...
def _build_parser() -> argparse.ArgumentParser:
  ...
  python main.py /data/study --keyword Control \
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Disable the combined summary table. Kept for compatibility; "
            "summaries are already disabled by default."
        ),
    )
    parser.add_argument(
        "--generate-measurement-summary",
        "--measurement-summary",
        dest="generate_measurement_summary",
        action="store_true",
        help="Create the combined measurement summary table.",
    )
    parser.add_argument(
        "--deconvolve",
        action="store_true",
        help=(
            "Run calibrated channel-wise 3D Richardson-Lucy before the selected "
            "analysis macro."
        ),
    )
    parser.add_argument(
        ...
        action="append",
        help=(
            "PSF TIFF Z-stack in channel order. Repeat once per channel or "
            "separate paths with semicolons. Providing --psf enables deconvolution."
        ),
    )
    parser.add_argument(
        "--deconvolution-iterations",
        type=int,
        default=DEFAULT_DECONVOLUTION_ITERATIONS,
        help="Richardson-Lucy iteration count (default: 10).",
    )
    parser.add_argument(
        "--deconvolution-folder",
        default=DEFAULT_DECONVOLUTION_FOLDER,
        help="Folder under the base path for float deconvolved stacks.",
    )
    parser.add_argument(
        "--deconvolution-memory-gb",
        type=int,
        default=DEFAULT_DECONVOLUTION_MEMORY_GB,
        help="Fiji heap used for deconvolution when supported by the launcher.",
    )
    parser.add_argument(
        "--deconvolution-timeout",
        type=int,
        default=DEFAULT_DECONVOLUTION_TIMEOUT_SECONDS,
        help="Maximum seconds allowed per deconvolution.",
    )
    parser.add_argument(
        "--roi-template",
        action="append",
    ...
    try:
        ...
        if args.validate:
            ...
            print(
                ...
                + (
                    validation["deconvolutionlab2_plugin"]
                    or "not installed"
                )
            )
            return 0
        ...
        psf_paths = _collect_psf_paths(args.psf or [])
        ...
        options = ProcessingOptions(
            ...
            generate_measurement_summary=(
                args.generate_measurement_summary
                and not args.skip_measurement_summary
            ),
            roi_search_templates=roi_templates,
            deconvolution_enabled=args.deconvolve or bool(psf_paths),
            deconvolution_psf_paths=psf_paths or None,
            deconvolution_iterations=args.deconvolution_iterations,
            deconvolution_folder=args.deconvolution_folder,
            deconvolution_memory_gb=args.deconvolution_memory_gb,
            deconvolution_timeout_seconds=args.deconvolution_timeout,
        )
