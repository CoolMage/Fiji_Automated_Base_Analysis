...
from fiji_automated_analysis.utils.general.deconvolution import (
    ...
    default_theoretical_psf_config,
    read_image_geometry_with_fiji,
    validate_theoretical_psf_config,
)
from fiji_automated_analysis.utils.general.fiji_utils import (
    detect_deconvolutionlab2_plugin,
    detect_ffmpeg_plugin,
    find_fiji,
)
from fiji_automated_analysis.utils.general.macro_builder import DEFAULT_MACRO_CODE
from fiji_automated_analysis.utils.general.measurement_summary_utils import detect_summary_naming_patterns
...
class FijiProcessorGUI:
    ...
    def _build_widgets(self) -> None:
        ...
        self.deconvolution_enabled_var = tk.BooleanVar(value=False)
        self.deconvolution_psf_mode_var = tk.StringVar(value=PSF_MODE_FILES)
        self.deconvolution_psf_var = tk.StringVar()
        self.deconvolution_theoretical_summary_var = tk.StringVar()
        self.deconvolution_theoretical_psf = default_theoretical_psf_config(3)
        self.deconvolution_iterations_var = tk.StringVar(
            value=str(DEFAULT_DECONVOLUTION_ITERATIONS)
        )
        self.deconvolution_memory_var = tk.StringVar(
            value=str(DEFAULT_DECONVOLUTION_MEMORY_GB)
        )
        ...
        self._checkbutton(
            deconvolution_frame,
            text="Deconvolve each Z-stack before the selected analysis macro",
            variable=self.deconvolution_enabled_var,
        ).grid(row=0, column=0, columnspan=3, sticky="w")
        tk.Label(
            deconvolution_frame,
            text=(
                "Use calibrated PSF TIFF stacks or calculate one synthetic "
                "AxialDiffractionSimulation PSF per channel in DeconvolutionLab2."
            ),
            justify=tk.LEFT,
            wraplength=760,
        ).grid(row=1, column=0, columnspan=3, sticky="w", pady=(4, 8))

        psf_mode_frame = tk.Frame(deconvolution_frame)
        psf_mode_frame.grid(row=2, column=0, columnspan=3, sticky="w")
        self._radiobutton(
            psf_mode_frame,
            text="PSF TIFF files",
            value=PSF_MODE_FILES,
            variable=self.deconvolution_psf_mode_var,
            command=self._update_deconvolution_psf_mode,
        ).pack(side=tk.LEFT)
        self._radiobutton(
            psf_mode_frame,
            text="Theoretical PSF (DL2)",
            value=PSF_MODE_THEORETICAL,
            variable=self.deconvolution_psf_mode_var,
            command=self._update_deconvolution_psf_mode,
        ).pack(side=tk.LEFT, padx=(15, 0))

        self.deconvolution_file_psf_frame = tk.Frame(deconvolution_frame)
        self.deconvolution_file_psf_frame.grid(
            row=3, column=0, columnspan=3, sticky="we", pady=(6, 0)
        )
        tk.Entry(
            self.deconvolution_file_psf_frame,
            textvariable=self.deconvolution_psf_var,
        ).grid(row=0, column=1, sticky="we", padx=(5, 5))
        tk.Label(
            self.deconvolution_file_psf_frame,
            text="PSF stacks (C1, C2, ...):",
        ).grid(row=0, column=0, sticky="w")
        tk.Button(
            self.deconvolution_file_psf_frame,
            text="Choose...",
            command=self._browse_psf_files,
        ).grid(row=0, column=2)
        self.deconvolution_file_psf_frame.grid_columnconfigure(1, weight=1)

        self.deconvolution_theoretical_psf_frame = tk.Frame(deconvolution_frame)
        self.deconvolution_theoretical_psf_frame.grid(
            row=3, column=0, columnspan=3, sticky="we", pady=(6, 0)
        )
        tk.Label(
            self.deconvolution_theoretical_psf_frame,
            textvariable=self.deconvolution_theoretical_summary_var,
            justify=tk.LEFT,
            anchor="w",
        ...
        tk.Button(
            self.deconvolution_theoretical_psf_frame,
            text="Configure...",
            command=self._open_theoretical_psf_window,
        ).grid(row=0, column=1, padx=(10, 0))
        self.deconvolution_theoretical_psf_frame.grid_columnconfigure(0, weight=1)

        parameter_frame = tk.Frame(deconvolution_frame)
        parameter_frame.grid(row=4, column=0, columnspan=3, sticky="w", pady=(8, 0))
        tk.Label(parameter_frame, text="RL iterations:").pack(side=tk.LEFT)
        tk.Spinbox(
            parameter_frame,
            from_=1,
            to=100,
            width=5,
            textvariable=self.deconvolution_iterations_var,
        ).pack(side=tk.LEFT, padx=(5, 15))
        tk.Label(parameter_frame, text="Fiji memory (GB):").pack(side=tk.LEFT)
        tk.Spinbox(
            parameter_frame,
            from_=2,
            to=256,
            width=5,
            textvariable=self.deconvolution_memory_var,
        ).pack(side=tk.LEFT, padx=(5, 0))
        deconvolution_frame.grid_columnconfigure(1, weight=1)
        self._update_theoretical_psf_summary()
    ...
    def _browse_psf_files(self) -> None:
        """Select calibrated PSF TIFF stacks in image-channel order."""

        initial = self.base_path_var.get().strip() or os.getcwd()
        paths = filedialog.askopenfilenames(
            parent=self.root,
            initialdir=initial,
            title="Select PSF stacks in C1, C2, ... order",
            filetypes=(
        ...
        if paths:
            self.deconvolution_psf_var.set("; ".join(paths))
            self.deconvolution_psf_mode_var.set(PSF_MODE_FILES)
            self.deconvolution_enabled_var.set(True)
            self._update_deconvolution_psf_mode()

    def _update_deconvolution_psf_mode(self) -> None:
        """Show only controls relevant to the selected PSF source."""

        if self.deconvolution_psf_mode_var.get() == PSF_MODE_THEORETICAL:
            self.deconvolution_file_psf_frame.grid_remove()
            self.deconvolution_theoretical_psf_frame.grid()
        else:
            self.deconvolution_theoretical_psf_frame.grid_remove()
            self.deconvolution_file_psf_frame.grid()

    def _update_theoretical_psf_summary(self) -> None:
        config = self.deconvolution_theoretical_psf
        self.deconvolution_theoretical_summary_var.set(
            "AxialDiffractionSimulation: "
            f"{len(config.channels)} channel(s), "
            f"{config.width} x {config.height} x {config.slices} px"
        )

    def _open_theoretical_psf_window(self) -> None:
        """Configure per-channel DL2 AxialDiffractionSimulation parameters."""

        window = tk.Toplevel(self.root)
        window.title("Theoretical PSF Parameters")
        ...
        outer = tk.Frame(window, padx=12, pady=12)
        outer.pack(fill=tk.BOTH, expand=True)

        tk.Label(
            outer,
            text="DeconvolutionLab2 model: AxialDiffractionSimulation",
        ...
        tk.Label(
            outer,
            text=(
                "The three channel parameters are native DL2 model parameters. "
                "They are not direct substitutes for NA, wavelength, or refractive index. "
                "The model is voxel-based; loaded calibration is used for compatibility checks."
            ),
            justify=tk.LEFT,
            wraplength=700,
            anchor="w",
        ...
        config = self.deconvolution_theoretical_psf
        reference_geometry = config.reference_geometry
        reference_image_path = config.reference_image_path
        common_frame = tk.LabelFrame(outer, text="Common PSF geometry", padx=10, pady=8)
        common_frame.pack(fill=tk.X)

        common_vars = {
            "channels": tk.StringVar(value=str(len(config.channels))),
            "width": tk.StringVar(value=str(config.width)),
            "height": tk.StringVar(value=str(config.height)),
            "slices": tk.StringVar(value=str(config.slices)),
            "intensity": tk.StringVar(value=f"{config.intensity:g}"),
            "center_x": tk.StringVar(value=f"{config.center_x:g}"),
            "center_y": tk.StringVar(value=f"{config.center_y:g}"),
            "center_z": tk.StringVar(value=f"{config.center_z:g}"),
        }
        common_fields = (
            ("Channels", "channels"),
            ("Width (px)", "width"),
            ...
            ("Z slices", "slices"),
            ("Intensity", "intensity"),
            ("Center X (0..1)", "center_x"),
            ("Center Y (0..1)", "center_y"),
            ("Center Z (0..1)", "center_z"),
        )
        for index, (label, key) in enumerate(common_fields):
            row = index // 4
            column = (index % 4) * 2
            tk.Label(common_frame, text=label).grid(
                row=row, column=column, sticky="w", padx=(0, 4), pady=3
            )
            tk.Entry(
                common_frame,
                width=9,
                textvariable=common_vars[key],
            ).grid(row=row, column=column + 1, sticky="w", padx=(0, 12), pady=3)

        channel_frame = tk.LabelFrame(
            outer,
            text="Per-channel DL2 parameters",
            padx=10,
            pady=8,
        )
        channel_frame.pack(fill=tk.BOTH, expand=True, pady=(10, 0))

        headers = ("Channel", "Pupil size", "Defocus factor", "Wave number (axial)")
        for column, text in enumerate(headers):
            tk.Label(channel_frame, text=text).grid(
                row=0, column=column, sticky="w", padx=(0, 10), pady=(0, 5)
            )

        existing_channels = list(config.channels)
        while len(existing_channels) < 7:
            existing_channels.append(TheoreticalPSFChannel())

        channel_vars: list[tuple[tk.StringVar, tk.StringVar, tk.StringVar]] = []
        channel_rows: list[list[tk.Widget]] = []
        for index in range(7):
            channel = existing_channels[index]
            variables = (
                tk.StringVar(value=f"{channel.pupil_size:g}"),
                tk.StringVar(value=f"{channel.defocus_factor:g}"),
                tk.StringVar(value=f"{channel.wave_number_axial:g}"),
            )
            channel_vars.append(variables)
            widgets: list[tk.Widget] = [
                tk.Label(channel_frame, text=f"C{index + 1}"),
                tk.Entry(channel_frame, width=14, textvariable=variables[0]),
                tk.Entry(channel_frame, width=14, textvariable=variables[1]),
                tk.Entry(channel_frame, width=18, textvariable=variables[2]),
            ]
            channel_rows.append(widgets)
            for column, widget in enumerate(widgets):
                widget.grid(
                    row=index + 1,
                    column=column,
                    sticky="we" if column else "w",
                    padx=(0, 10),
                    pady=3,
                )

        def update_channel_rows(*_args: object) -> None:
            try:
                count = max(1, min(7, int(common_vars["channels"].get())))
            except ValueError:
                count = 1
            for index, widgets in enumerate(channel_rows):
        ...
        common_vars["channels"].trace_add("write", update_channel_rows)
        update_channel_rows()

        geometry_status_var = tk.StringVar()
        ...
        def format_reference_status(
            ...
            if geometry is None:
                return (
                    "Loads channel count, image dimensions, and voxel sampling "
                    "from the first matching image. PSF dimensions stay unchanged."
                )

            calibration = (
                f"{geometry.pixel_width_um:g} x "
                f"{geometry.pixel_height_um:g} x "
                f"{geometry.voxel_depth_um:g} um"
                if all(
                    value is not None
                    for value in (
                        geometry.pixel_width_um,
                        geometry.pixel_height_um,
                        geometry.voxel_depth_um,
                    )
                )
                else "not available"
            )
            warnings: list[str] = []
            try:
                psf_width = int(common_vars["width"].get())
                psf_height = int(common_vars["height"].get())
                psf_slices = int(common_vars["slices"].get())
                if (
                    psf_width > geometry.width
                    or psf_height > geometry.height
                    or psf_slices > geometry.slices
                ):
                    warnings.append(
                        "PSF dimensions exceed the image; reduce them before Apply."
                    )
            except ValueError:
                pass
            if geometry.slices < 7:
                warnings.append(
                    f"The image has only {geometry.slices} Z slices; "
                    "3D deconvolution may be poorly constrained."
                )
            ...
            status = (
                f"{source_name}: C={geometry.channels}; "
                f"image={geometry.width} x {geometry.height} x {geometry.slices}; "
                f"voxel={calibration}. PSF dimensions were not changed."
            )
            if warnings:
                status += " " + " ".join(warnings)
            return status

        geometry_status_var.set(
            format_reference_status(
                reference_geometry,
                reference_image_path,
            )
        )

        def finish_geometry_load(
            *,
            geometry: Optional[ImageGeometry] = None,
            source_path: str = "",
            error: str = "",
        ) -> None:
            nonlocal reference_geometry, reference_image_path

            if not window.winfo_exists():
                return
            load_geometry_button.configure(state=tk.NORMAL)
            if error:
                geometry_status_var.set("Geometry was not loaded.")
                messagebox.showerror(
                    "Load geometry from image",
                    error,
                    parent=window,
                )
                return
            ...
            common_vars["channels"].set(str(geometry.channels))
            geometry_status_var.set(
                format_reference_status(geometry, source_path)
            )

        def load_geometry_from_matching_image() -> None:
            base_path = self.base_path_var.get().strip()
            keywords = self._collect_listbox_values(self.keyword_listbox)
            if not base_path:
                messagebox.showwarning(
                    "Load geometry from image",
                    "Select the base directory first.",
                    parent=window,
                )
                return
            ...
            load_geometry_button.configure(state=tk.DISABLED)
            geometry_status_var.set("Searching matching images and reading metadata...")
            ...
            def worker() -> None:
                try:
                    ...
                    documents = processor.find_documents_by_keyword(
                        base_path,
                        keywords,
                        search_options,
                    )
                    if not documents:
                        raise ValueError(
                            "No image matches the selected directory, keywords, "
                            "secondary filter, and supported file formats."
                        )
                    document = min(
                        documents,
                        key=lambda item: item.file_path.casefold(),
                    )
                    geometry = read_image_geometry_with_fiji(
                        processor.fiji_path,
                        document.file_path,
                    )
                    if not 1 <= geometry.channels <= 7:
                        raise ValueError(
                            "The selected image has "
                            f"{geometry.channels} channels; deconvolution supports 1 to 7."
                        )
                    if geometry.slices < 2:
                        raise ValueError(
                            "The selected image is 2D; deconvolution requires a Z-stack."
                        )
                    if geometry.frames != 1:
                        raise ValueError(
                            "The selected image is a time series with "
                            f"{geometry.frames} frames. Export one time point per Z-stack."
                        )
                except Exception as exc:
                    result_queue.put(("error", str(exc), ""))
                    return

                result_queue.put(("success", geometry, document.file_path))

            def poll_worker_result() -> None:
                try:
                    if not window.winfo_exists():
                        return
                except tk.TclError:
                ...
                try:
                    status, payload, source_path = result_queue.get_nowait()
                except queue.Empty:
                    window.after(100, poll_worker_result)
                    return

                if status == "error":
                    finish_geometry_load(error=str(payload))
                else:
                    finish_geometry_load(
                        geometry=payload,
        ...
        load_geometry_button = tk.Button(
            common_frame,
            text="Load channels + calibration",
            command=load_geometry_from_matching_image,
        )
        load_geometry_button.grid(
            row=2,
            column=0,
            columnspan=3,
            sticky="w",
        ...
        tk.Label(
            common_frame,
            textvariable=geometry_status_var,
            justify=tk.LEFT,
            anchor="w",
            wraplength=650,
        ).grid(
        ...
        def save_configuration() -> None:
            try:
                channel_count = int(common_vars["channels"].get())
                channels = tuple(
                    TheoreticalPSFChannel(
                        pupil_size=float(channel_vars[index][0].get()),
                        defocus_factor=float(channel_vars[index][1].get()),
                        wave_number_axial=float(channel_vars[index][2].get()),
                    )
                    for index in range(channel_count)
                )
                updated = TheoreticalPSFConfig(
                    width=int(common_vars["width"].get()),
                    height=int(common_vars["height"].get()),
                    slices=int(common_vars["slices"].get()),
                    intensity=float(common_vars["intensity"].get()),
                    center_x=float(common_vars["center_x"].get()),
                    center_y=float(common_vars["center_y"].get()),
                    center_z=float(common_vars["center_z"].get()),
                    channels=channels,
                    reference_geometry=reference_geometry,
                    reference_image_path=reference_image_path,
                )
                validate_theoretical_psf_config(updated)
            except (ValueError, IndexError) as exc:
                messagebox.showerror(
                    "Invalid theoretical PSF parameters",
                    str(exc),
                    parent=window,
                )
                return

            self.deconvolution_theoretical_psf = updated
            self.deconvolution_psf_mode_var.set(PSF_MODE_THEORETICAL)
            self.deconvolution_enabled_var.set(True)
            self._update_theoretical_psf_summary()
            self._update_deconvolution_psf_mode()
            window.destroy()
    ...
    def _validate_setup(self) -> None:
        try:
            ...
            if fiji_path:
                ...
                deconvolution_ok = detect_deconvolutionlab2_plugin(fiji_path)
                self._log(
                    f"DeconvolutionLab2 plugin available: {deconvolution_ok}"
                )
            details = [
                ...
                + (
                    str(validation["deconvolutionlab2_plugin"])
                    if validation.get("deconvolutionlab2_plugin")
                    else "not installed"
                ),
            ]
            self._log("\n".join(details))
    ...
    def _run_processing(self) -> None:
        ...
        try:
            ...
            messagebox.showerror("Processing options", str(exc))
            return
        processor = self._get_processor()
        args = dict(
            base_path=base_path,
    ...
    def _gather_processing_options(self) -> ProcessingOptions:
        ...
        deconvolution_enabled = self.deconvolution_enabled_var.get()
        deconvolution_psf_mode = self.deconvolution_psf_mode_var.get()
        psf_paths = [
            path.strip()
            for path in self.deconvolution_psf_var.get().split(";")
            if path.strip()
        ]
        if (
            deconvolution_enabled
            and deconvolution_psf_mode == PSF_MODE_FILES
            and not psf_paths
        ):
            raise ValueError(
                "Select one calibrated PSF TIFF Z-stack per image channel."
            )
        theoretical_psf = None
        if deconvolution_enabled and deconvolution_psf_mode == PSF_MODE_THEORETICAL:
            theoretical_psf = validate_theoretical_psf_config(
                self.deconvolution_theoretical_psf
            )
        try:
            deconvolution_iterations = int(
                self.deconvolution_iterations_var.get().strip()
            )
            deconvolution_memory_gb = int(
                self.deconvolution_memory_var.get().strip()
            )
        except ValueError as exc:
            raise ValueError(
                "Deconvolution iterations and memory must be whole numbers."
            ) from exc
        ...
        options = ProcessingOptions(
            ...
            deconvolution_psf_mode=deconvolution_psf_mode,
            deconvolution_psf_paths=(
                (psf_paths or None)
                if deconvolution_psf_mode == PSF_MODE_FILES
                else None
            ),
            deconvolution_theoretical_psf=theoretical_psf,
            deconvolution_iterations=deconvolution_iterations,
            deconvolution_memory_gb=deconvolution_memory_gb,
        )
